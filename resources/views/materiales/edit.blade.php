<x-layouts.app :title="__('Editar Material')">
    <div class="p-6">
        <div class="max-w-2xl mx-auto">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white mb-6">Editar Material</h1>
            
            <form action="{{ route('materiales.update', $materiale) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div>
                    <label for="nombre" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre</label>
                    <input type="text" name="nombre" id="nombre" required value="{{ old('nombre', $materiale->nombre) }}"
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', $materiale->descripcion) }}</textarea>
                </div>

                <div>
                    <label for="cantidad" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Cantidad</label>
                    <input type="number" name="cantidad" id="cantidad" required value="{{ old('cantidad', $materiale->cantidad) }}" min="0"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <select name="taller_id" id="taller_id" required
                        ...
                        @foreach($talleres as $taller)
                            <option value="{{ $taller->id }}" {{ (old('taller_id', $materiale->taller_id) == $taller->id) ? 'selected' : '' }}>
                                {{ $taller->nombre }}
                            </option>
                        @endforeach
                    </select>
